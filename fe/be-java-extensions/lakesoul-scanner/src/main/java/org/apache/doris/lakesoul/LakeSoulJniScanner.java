...
import org.apache.doris.lakesoul.arrow.ArrowUtils;

import java.io.IOException;
import java.util.List;

public class LakeSoulJniScanner extends JniScanner {

    private final DBManager dbManager = new DBManager();

    public LakeSoulJniScanner(int batchSize) {
        TableInfo tableInfo =
            dbManager.getTableInfoByNameAndNamespace("tableName", "namespace");
        String tableSchema = tableInfo.getTableSchema();

        org.apache.arrow.vector.types.pojo.Schema arrowSchema = null;

        try {
            arrowSchema = Schema.fromJSON(tableSchema);
        } catch (IOException e) {
